...
def _smoke_config(**overrides) -> SmokeConfig:
    values = {
        ...
        "timeout_s": 45.0,
        "prompt": "Reply with exactly: GDEC_SMOKE_PONG",
        "claude_bin": "claude",
        "worker_id": "main",
        "settings": _settings(),
    }
...
def test_provider_smoke_models_cover_configured_providers_independent_of_model_mapping(
    ...
    monkeypatch.delenv("GDEC_SMOKE_MODEL_DEEPSEEK", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
...
def test_wafer_provider_configuration_uses_api_key(monkeypatch) -> None:
    monkeypatch.delenv("GDEC_SMOKE_MODEL_WAFER", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_provider_smoke_model_override_accepts_model_name_without_prefix(
    ...
    monkeypatch.setenv("GDEC_SMOKE_MODEL_DEEPSEEK", "deepseek-reasoner")
    config = _smoke_config(
        settings=_settings(
            deepseek_api_key="deepseek-key",
            ollama_base_url="",
    ...
    assert models[0].source == "GDEC_SMOKE_MODEL_DEEPSEEK"


def test_provider_smoke_model_override_accepts_owner_model_name(
    monkeypatch,
) -> None:
    monkeypatch.setenv("GDEC_SMOKE_MODEL_NVIDIA_NIM", "z-ai/glm4.7")
    config = _smoke_config(
        settings=_settings(
    ...
    assert models[0].source == "GDEC_SMOKE_MODEL_NVIDIA_NIM"


def test_provider_smoke_model_override_rejects_wrong_provider_prefix(
    monkeypatch,
) -> None:
    monkeypatch.setenv("GDEC_SMOKE_MODEL_DEEPSEEK", "ollama/llama3.1")
    config = _smoke_config(
        settings=_settings(
            deepseek_api_key="deepseek-key",
            ollama_base_url="",
    ...
    try:
        ...
        assert "GDEC_SMOKE_MODEL_DEEPSEEK" in str(exc)
    else:
...
def test_provider_smoke_includes_local_provider_when_model_mapping_uses_it(
    ...
    monkeypatch.delenv("GDEC_SMOKE_MODEL_OLLAMA", raising=False)
    config = _smoke_config()
...
def test_nvidia_nim_cli_models_override_and_append() -> None:
    refs = nvidia_nim_cli_model_refs(
        {
            "GDEC_SMOKE_NIM_MODELS": "z-ai/glm-5.1,nvidia_nim/custom/model",
            "GDEC_SMOKE_NIM_EXTRA_MODELS": "moonshotai/kimi-k2.6,z-ai/glm-5.1",
        }
    )
    ...
    assert refs["nvidia_nim/z-ai/glm-5.1"] == "GDEC_SMOKE_NIM_MODELS"
    assert refs["nvidia_nim/moonshotai/kimi-k2.6"] == ("GDEC_SMOKE_NIM_EXTRA_MODELS")


def test_nvidia_nim_cli_models_reject_empty_override() -> None:
    try:
        nvidia_nim_cli_model_refs({"GDEC_SMOKE_NIM_MODELS": " , "})
    except ValueError as exc:
        assert "GDEC_SMOKE_NIM_MODELS" in str(exc)
    else:
...
def test_nvidia_nim_cli_models_reject_wrong_provider_prefix() -> None:
    try:
        nvidia_nim_cli_model_refs({"GDEC_SMOKE_NIM_MODELS": "open_router/model"})
    except ValueError as exc:
...
def test_smoke_config_returns_nvidia_nim_cli_provider_models(monkeypatch) -> None:
    monkeypatch.delenv("GDEC_SMOKE_NIM_MODELS", raising=False)
    monkeypatch.delenv("GDEC_SMOKE_NIM_EXTRA_MODELS", raising=False)
    config = _smoke_config(
        settings=_settings(
...
def test_openrouter_free_cli_models_reject_empty_override() -> None:
    try:
        openrouter_free_cli_model_refs({"GDEC_SMOKE_OPENROUTER_FREE_MODELS": " , "})
    except ValueError as exc:
        assert "GDEC_SMOKE_OPENROUTER_FREE_MODELS" in str(exc)
    else:
...
def test_openrouter_free_cli_models_reject_wrong_provider_prefix() -> None:
    try:
        openrouter_free_cli_model_refs(
            {"GDEC_SMOKE_OPENROUTER_FREE_MODELS": "nvidia_nim/model"}
        )
    except ValueError as exc:
...
def test_smoke_config_returns_openrouter_free_cli_provider_models(monkeypatch) -> None:
    monkeypatch.delenv("GDEC_SMOKE_OPENROUTER_FREE_MODELS", raising=False)
    monkeypatch.delenv("GDEC_SMOKE_OPENROUTER_FREE_EXTRA_MODELS", raising=False)
    config = _smoke_config(
        settings=_settings(
