...
The second command skips everything unless `GDEC_LIVE_SMOKE=1` is set, but still
writes skip entries to `.smoke-results/`.

## Product Smoke Run

```powershell
$env:GDEC_LIVE_SMOKE = "1"
uv run pytest smoke -n 0 -s --tb=short
```
...
catalog/docs and can be overridden with `GDEC_SMOKE_MODEL_<PROVIDER>`, for example
`GDEC_SMOKE_MODEL_DEEPSEEK=deepseek-v4-pro` (or `deepseek-v4-flash`). If no provider smoke model is
configured, live product smoke fails as `missing_env` unless you explicitly set
`GDEC_ALLOW_NO_PROVIDER_SMOKE=1`.
...
| `cli` | `gdec-init`, server entrypoint, Claude CLI adaptive thinking, session cleanup | Claude CLI binary and provider only for real CLI |
...
| `providers` | multi-turn text, adaptive thinking history, tools, disconnect, errors | configured providers, optional `GDEC_SMOKE_MODEL_*` |
...
| `voice` | generated WAV through local Whisper or NVIDIA NIM transcription | `VOICE_NOTE_ENABLED=true`, `GDEC_SMOKE_RUN_VOICE=1` |

## Examples

```powershell
$env:GDEC_LIVE_SMOKE = "1"
$env:GDEC_SMOKE_PROVIDER_MATRIX = "open_router,nvidia_nim,deepseek,lmstudio,llamacpp,ollama"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:GDEC_LIVE_SMOKE = "1"
$env:GDEC_SMOKE_TARGETS = "ollama"
...
```powershell
$env:GDEC_LIVE_SMOKE = "1"
$env:GDEC_SMOKE_TARGETS = "telegram,discord,voice"
$env:GDEC_SMOKE_RUN_VOICE = "1"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:GDEC_LIVE_SMOKE = "1"
$env:GDEC_SMOKE_TARGETS = "nvidia_nim_cli"
$env:GDEC_SMOKE_NIM_MODELS = "z-ai/glm-5.1,moonshotai/kimi-k2.6,minimaxai/minimax-m2.7,nvidia/nemotron-3-super-120b-a12b,deepseek-ai/deepseek-v4-pro,deepseek-ai/deepseek-v4-flash"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:GDEC_LIVE_SMOKE = "1"
$env:GDEC_SMOKE_TARGETS = "openrouter_free_cli"
$env:GDEC_SMOKE_OPENROUTER_FREE_MODELS = "nvidia/nemotron-3-super-120b-a12b:free,openai/gpt-oss-120b:free,minimax/minimax-m2.5:free,inclusionai/ring-2.6-1t:free,poolside/laguna-m.1:free"
uv run pytest smoke/product -n 0 -s --tb=short
```

```powershell
$env:GDEC_LIVE_SMOKE = "1"
$env:GDEC_SMOKE_TARGETS = "messaging,config,extensibility"
uv run pytest smoke/product -n 0 -s --tb=short
```

## Environment

- `GDEC_ENV_FILE`: explicit dotenv path for startup/config scenarios.
- `GDEC_LIVE_SMOKE=1`: enables live smoke execution.
- `GDEC_ALLOW_NO_PROVIDER_SMOKE=1`: permits no-provider live smoke for harness work.
- `GDEC_SMOKE_TARGETS`: comma-separated targets, or `all`.
- `GDEC_SMOKE_PROVIDER_MATRIX`: comma-separated provider prefixes to require.
- `GDEC_SMOKE_MODEL_NVIDIA_NIM`, `GDEC_SMOKE_MODEL_OPEN_ROUTER`,
  `GDEC_SMOKE_MODEL_DEEPSEEK`, `GDEC_SMOKE_MODEL_LMSTUDIO`,
  `GDEC_SMOKE_MODEL_LLAMACPP`, `GDEC_SMOKE_MODEL_OLLAMA`: optional per-provider
  smoke model overrides. Values may include the provider prefix or just the model
  name for that provider.
- `GDEC_SMOKE_NIM_MODELS`: optional comma-separated NVIDIA NIM CLI matrix models
  that replace the default characterization set.
- `GDEC_SMOKE_NIM_EXTRA_MODELS`: optional comma-separated NVIDIA NIM CLI matrix
  models appended to the default or replacement set.
- `GDEC_SMOKE_OPENROUTER_FREE_MODELS`: optional comma-separated OpenRouter free
  CLI matrix models that replace the default characterization set.
- `GDEC_SMOKE_OPENROUTER_FREE_EXTRA_MODELS`: optional comma-separated OpenRouter
  free CLI matrix models appended to the default or replacement set.
- `GDEC_SMOKE_TIMEOUT_S`: per-request/subprocess timeout, default `45`.
- `GDEC_SMOKE_CLAUDE_BIN`: Claude CLI executable name, default `claude`.
- `GDEC_SMOKE_TELEGRAM_CHAT_ID`: Telegram chat/user ID for send/edit/delete.
- `GDEC_SMOKE_DISCORD_CHANNEL_ID`: Discord channel ID for send/edit/delete.
- `GDEC_SMOKE_INTERACTIVE=1`: enables manual inbound Telegram/Discord checks.
- `GDEC_SMOKE_RUN_VOICE=1`: allows voice transcription backends to load/run.
...
`uv run`, so Windows does not try to replace `gde-code.exe` while it is
locked.
...
- `target_disabled`: skipped because `GDEC_SMOKE_TARGETS` intentionally selected
  a different target.
...
explicitly selected a provider in `GDEC_SMOKE_PROVIDER_MATRIX`;
